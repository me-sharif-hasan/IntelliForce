...
public class Apex {
    ...
    public String getAstJson(String sourceCode){
        StringWriter writer = new StringWriter();
        try {
            getAST(false, true, new StringReader(sourceCode), writer);
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public static void main(String[] args) throws ParseException, IOException {
        ...
        // Single-line source code as input
        String sourceCode = "public class Test { public void method() {if(true){System.debug('hello');integer x=5+2;}} }";

        // Create input and output streams
        try (Reader apexReader = new StringReader(sourceCode);
             Writer writer = new StringWriter()) {

            // Call the getAST method
            getAST(false, true, apexReader, writer);
